...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 18); // proof of stake target limit since 20 June 2013, equal to 0.03125  proof of stake difficulty
CBigNum bnProofOfStakeHardLimit(~uint256(0) >> 18); // disabled temporarily, will be used in the future to fix minimal proof of stake difficulty at 0.25
...
const string strMessageMagic = "RajmudraCoin Signed Message:\n";
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if(IsCoinStake())
    {
        // Enforce 0.00 as minimum fee for coinstake
        nMinTxFee = 0;
        nMinRelayTxFee = 0;
    }
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (nActualSpacing > 4 * nTargetSpacing)
		nActualSpacing = 4 * nTargetSpacing;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("RajmudraCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in RajmudraCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // RajmudraCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RajmudraCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "raj genesis";
        CTransaction txNew;
        txNew.nTime = 1505914819;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1505914819;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 437460; // !fTestNet ? 1575379 : 46534;
