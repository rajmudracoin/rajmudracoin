...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("RajmudraCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RajmudraCoinTEST2\r");
            Send(hSocket, "WHO #RajmudraCoinTEST2\r");
        } else {
            // randomly join #RajmudraCoin00-#RajmudraCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #RajmudraCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RajmudraCoin%02d\r", channel_number).c_str());
        }
