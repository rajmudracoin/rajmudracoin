RajmudraCoin-qt: Qt4 GUI for RajmudraCoin
===============================
...
Alternatively, install Qt Creator and open the `RajmudraCoin-qt.pro` file.

An executable named `RajmudraCoin-qt` will be built.
...
A warning for people using the *static binary* version of RajmudraCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of RajmudraCoin is linked against libdb5.3.
...
installed causes RajmudraCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
